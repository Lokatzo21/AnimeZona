...
class AnimeOnlineProvider extends BaseProvider {
            ...
            for (const server of serversToExtract) {
                ...
                if (!videoUrl) {
                    ...
                    if (videoUrl.includes('.mp4')) {
                        this.log(`[IA] Verificando integridad del link MP4 de ${server.name}...`, 'info');
                        try {
                            const fetchOptions = { 
                                method: 'HEAD', 
                                headers: { 
                            ...
                            if (response.status === 404 || response.status === 403 || response.status === 410 || response.status === 500) {
                                ...
                                if (contentLength && parseInt(contentLength, 10) < 100000) { 
                                    isAlive = false;
                                    this.log(`[ALERTA IA] El enlace mp4 es un archivo inusualmente pequeño (${contentLength} bytes). Posible error encubierto.`, 'error');
                                } else {
                        ...
                        this.log(`[IA] Analizando reproductor embebido de ${server.name} en pantalla...`, 'info');
                        try {
                            let isDeadFrame = false;
                            for (const frame of targetPage.frames()) {
                                if (frame.url() === videoUrl || frame.url().includes(videoUrl)) {
                                    // Click play button inside iframe to trigger error message if it's hidden
                                    await frame.evaluate(() => {
                                        const playBtn = document.querySelector('#vid_play, .vjs-big-play-button, .play-button');
                                        if (playBtn) playBtn.click();
                                    }).catch(() => {});
                                    
                                    await this.delay(2000); // Wait for error to load

                                    const text = await frame.evaluate(() => document.body ? document.body.innerText.toLowerCase() : '').catch(() => '');
                                    const html = await frame.evaluate(() => document.body ? document.body.innerHTML.toLowerCase() : '').catch(() => '');
                                    ...
                                    const foundBadPhrase = badPhrases.find(phrase => fullText.includes(phrase));
                                    if (foundBadPhrase) {
                                        isDeadFrame = true;
                                        this.log(`[ALERTA IA] Archivo borrado en ${server.name} (Razón: "${foundBadPhrase}").`, 'error');
                                    }
                                    break;
                                }
                            }
                            ...
                            if (isDeadFrame) {
                                ...
                                this.log(`[IA] Reproductor de ${server.name} verificado en pantalla y parece funcional.`, 'success');
                            }
                        } catch (err) {
                            this.log(`[IA] Error al verificar frame de ${server.name}, asumiendo vivo.`, 'info');
                        }
                    }

                    if (!isAlive) {
                        this.log(`[IA] Descartando la opción de ${server.name} para evitar almacenar un link muerto.`, 'warning');
                        continue;
                    }
  ...
  async scrapeSingle(episodeUrl, title, episodeNumber) {
    ...
    this.log(`Iniciando extracción individual del episodio ${currentEpisodeNumber}...`, 'info');
    
    // Agrupar servidores existentes para este episodio
    ...
    for (let j = 0; j < 30; j++) {
       ...
       for (const p of pages) {
         try {
           if (p.url().includes('/episodio/') && p.url() === episodeUrl) {
             targetPage = p;
             isOnEpPage = true;
             break;
           }
    ...
    if (!isOnEpPage) {
        throw new Error(`No se pudo cargar el episodio individual. Asegúrate de que la URL sea correcta.`);
    }
    ...
    if (!targetLanguage) {
        throw new Error('No se encontraron idiomas en este episodio.');
    }
    ...
    if (successCount > 0) {
        this.log(`✅ Episodio ${currentEpisodeNumber} individual completado (${successCount} servidores guardados).`, 'success');
    } else {
        this.log(`❌ No se pudo guardar ningún servidor para el episodio ${currentEpisodeNumber} individual.`, 'error');
    }
  }
}
